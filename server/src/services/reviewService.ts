...
type RawScheduledRow = {
  id: number; deck_id: number; front: string; back: string; created_at: number;
  card_id: number; stability: number; difficulty: number; elapsed_days: number;
  scheduled_days: number; reps: number; lapses: number; state: string;
  due_at: number; last_review_at: number | null;
};
...
function rowToCardWithSchedule(r: RawScheduledRow): CardWithSchedule {
  return {
    id: r.id, deckId: r.deck_id, front: r.front, back: r.back, createdAt: r.created_at,
    scheduling: {
      cardId: r.card_id, stability: r.stability, difficulty: r.difficulty,
      ...
      dueAt: r.due_at, lastReviewAt: r.last_review_at,
    },
  };
}
...
export function submitReview({ cardId, rating }: SubmitReviewRequest): CardWithSchedule {
  ...
  const current = {
    ...
    elapsedDays: sched.elapsedDays, scheduledDays: sched.scheduledDays,
    reps: sched.reps, lapses: sched.lapses,
    state: sched.state as 'new' | 'learning' | 'review' | 'relearning',
    dueAt: sched.dueAt, lastReviewAt: sched.lastReviewAt,
  };
  ...
  db.update(cardScheduling)
    .set({
      ...
      elapsedDays: Math.round(next.elapsedDays), scheduledDays: next.scheduledDays,
      reps: next.reps, lapses: next.lapses, state: next.state,
      dueAt: next.dueAt, lastReviewAt: next.lastReviewAt,
    })
    .where(eq(cardScheduling.cardId, cardId))
    .run();

  db.insert(reviews).values({
    cardId, rating, state: current.state, dueAt: current.dueAt,
    reviewedAt: now, scheduledDays: next.scheduledDays,
  }).run();

  const card = db.select().from(cards).where(eq(cards.id, cardId)).get()!;
  return {
    id: card.id, deckId: card.deckId, front: card.front, back: card.back, createdAt: card.createdAt,
    scheduling: {
      cardId, stability: next.stability, difficulty: next.difficulty,
      elapsedDays: Math.round(next.elapsedDays), scheduledDays: next.scheduledDays,
      reps: next.reps, lapses: next.lapses, state: next.state,
      dueAt: next.dueAt, lastReviewAt: next.lastReviewAt,
    },
